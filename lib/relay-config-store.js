...
function buildSupabaseStore({ supabaseUrl, supabaseServiceRoleKey }) {
  ...
  // Step 5.5: 5-second TTL cache + in-flight dedup so 100 concurrent
  // requireAuthAny() calls fan into 1 Supabase round-trip, not 100.
  // On fetch failure we serve the previous cached value if any (no poisoning),
  // and throw only if there is no prior cache to fall back to.
  ...
  async function loadConfig() {
    const now = Date.now();
    if (cachedConfig && cachedExpiresAt > now) {
      return cachedConfig;
    }
    if (inFlightLoad) return inFlightLoad;
    inFlightLoad = (async () => {
      try {
        console.log('[relay-config] loadConfig fetch (cache miss/expired)');
        const config = await loadConfigUncached();
        cachedConfig = config;
        cachedExpiresAt = Date.now() + CACHE_TTL_MS;
        return config;
      } catch (err) {
        if (cachedConfig) {
          console.warn(`[relay-config] loadConfig fetch failed, serving stale cache: ${err.message}`);
          return cachedConfig;
        }
        throw err;
      } finally {
  ...
  async function replaceConfig(nextConfig) {
    // Admin operation — read fresh, not cached, so the diff is accurate.
    const currentConfig = await loadConfigUncached();
